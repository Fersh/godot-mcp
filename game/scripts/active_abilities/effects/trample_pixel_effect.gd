...
# Trample - T2 Charge with ground damage and multiple impacts

var pixel_size := 4
var duration := 0.6
var elapsed := 0.0

# Multiple impact points along charge path
var impacts := []
var num_impacts := 4
...
func _ready() -> void:
	# Initialize impacts along a line (charge path)
	for i in range(num_impacts):
		var x_offset = (i - num_impacts/2.0) * 25
		impacts.append({
			"pos": Vector2(x_offset, 0),
			"radius": 0.0,
			"max_radius": randf_range(25, 40),
			"alpha": 1.0,
			"delay": i * 0.08
		})
	...
	for i in range(num_dust):
		dust_trail.append({
			"pos": Vector2(randf_range(-60, 60), randf_range(-10, 10)),
			"velocity": Vector2(randf_range(-30, 30), randf_range(-50, -20)),
	...
	for i in range(num_debris):
		...
		var base_pos = Vector2((impact_idx - num_impacts/2.0) * 25, 0)
		var angle = randf() * TAU
		debris.append({
			"pos": base_pos,
...
func _process(delta: float) -> void:
	...
	for impact in impacts:
		if elapsed > impact.delay:
			var t = (elapsed - impact.delay) / 0.3
	...
	for crack in cracks:
		if elapsed > crack.delay:
			var t = (elapsed - crack.delay) / 0.25
	...
	for dust in dust_trail:
		...
		dust.velocity *= 0.95
		dust.alpha = max(0, 0.6 - progress * 0.8)
		dust.size += delta * 10
...
func _draw() -> void:
	...
	for dust in dust_trail:
		if dust.alpha > 0:
			var color = Color(0.6, 0.5, 0.4, dust.alpha * 0.5)
	...
	for crack in cracks:
		if crack.length > 0:
			...
			var end_pos = crack.origin + Vector2(cos(crack.angle), sin(crack.angle)) * crack.length
			_draw_pixel_line(crack.origin, end_pos, color)

	# Draw impact rings
	for impact in impacts:
		if impact.alpha > 0 and impact.radius > 0:
			# Outer ring (brown)
			var outer_color = Color(0.5, 0.4, 0.3, impact.alpha * 0.7)
			_draw_pixel_ring(impact.pos, impact.radius, outer_color, 6)
			# Inner flash
			var inner_color = Color(0.8, 0.7, 0.5, impact.alpha * 0.5)
